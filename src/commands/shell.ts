...
export async function shell(cmd: Array<string>, senderNumber: string, m: any): Promise<void> {
  if (cmd.length == 0) {
  ...
  try {
    ...
    console.error(error);
    utils.reply("There is an error when executing shell command", senderNumber, m);
  }
}
